...
:repo:`firmware/python/simple_zcu111_example/_Root.py`,
:repo:`firmware/python/simple_zcu111_example/_RFSoC.py`, and
:repo:`firmware/python/simple_zcu111_example/_Application.py`.

Hierarchy
---------

* ``Root`` (``pr.Root``, see :repo:`firmware/python/simple_zcu111_example/_Root.py`)
  ...
  * ``adcProcessor[0..7]``, ``dacProcessor[0..7]`` (host-side
...
``Root.start()`` (in
:repo:`firmware/python/simple_zcu111_example/_Root.py`) executes the
following ordered sequence after the base ``pr.Root.start()`` runs:

1. TCP connection setup (memory map + RFDC + ring-buffer streams).
2. Stream wiring (8 ADC + 8 DAC ring buffer paths chained to ``DataWriter``
   and to per-channel host-side processors via ``>>``).
...
:repo:`firmware/python/simple_zcu111_example/__init__.py`:
